...
Terraform module for the AWS resources backing Meryl Green Designs: S3 + CloudFront for the static frontend, Lambda + Function URL for the backend, ACM + Route 53 for DNS/TLS, GitHub OIDC for CI auth.
...
- **OIDC only for CI auth.** Don't introduce long-lived AWS access keys. The GitHub Actions role (`github_oidc.tf`) is trust-policied to the `main` branch of the configured repo.
