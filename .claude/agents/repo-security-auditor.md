...
Cross-cutting:
- **Secrets are SOPS-encrypted and live in a SEPARATE private repo.** The encrypted `terraform.tfvars.sops` + `.env.sops` are committed in the sibling private repo `Absence0760/infra-secrets` (under `meryl-green-designs/`), NOT in this public repo (relocated 2026-06-24). Plaintext siblings (`backend/.env`, `infra/terraform.tfvars`) are gitignored scratch; `.gitignore` also blocks `*.sops`. KMS key alias: `alias/meryl-green-designs-sops` in `af-south-1`. Any `*.sops` file appearing under this repo is a finding.
...
|---|---|---|
| `secrets` | No `*.sops` blob committed in THIS repo (secrets live in the private `infra-secrets` repo); plaintext `.env`/`.tfvars` never in git history; server-only env vars never referenced from a non-server frontend path; GitHub Actions `env:` blocks reference `${{ secrets.X }}` not literals; no AWS access keys anywhere | `.gitignore` (the `*.sops` block), `../infra-secrets/meryl-green-designs/` (the encrypted source), `.github/workflows/`, `frontend/src/`, root `package.json` |
