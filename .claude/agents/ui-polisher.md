...
2. **Vitest** (mandatory if you touched cart logic, the safeHttpUrl helper, or the sanity wrapper): `pnpm frontend test`. The `.svelte` files themselves are not unit-testable per `frontend/CLAUDE.md` — testable logic lives in plain `.ts` siblings.
3. **Playwright** (optional but recommended if the target has e2e coverage): grep `playwright/tests` for selectors used in your redesigned page. If selectors moved, update the test to match the new contract. The Playwright suite needs LocalStack + the test-e2e Sanity dataset (`pnpm dev:db:up` first); if that's not running, skip this step and flag it in your report — don't try to spin up the dev infrastructure yourself.
