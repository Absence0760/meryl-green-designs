...
8. **Public asset leak.**
   - Search `frontend/static/` for any file containing key shapes — `sk_`, `rs_` (Resend keys), `key-`, `Bearer `, hex strings ≥ 32 chars. Pasted-into-asset-config leaks have happened.

9. **Git history pickaxe.**
   - `git log --all -S 'RESEND_API_KEY' -S 'SANITY_API_TOKEN' -S 'PAYFAST_PASSPHRASE' -S 'AWS_ACCESS_KEY' -S 'sk-' --source --pretty=fuller`
