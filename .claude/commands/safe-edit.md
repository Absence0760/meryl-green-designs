...
**Right fit:**
- Security-sensitive changes — anything touching the PayFast ITN handler, the Sanity webhook handler, CORS config, rate limiting, the email pipeline (especially the banking-details regression test), or the SOPS-encrypted secrets files.
- Order-flow changes — `POST /orders`, `GET /orders/:ref`, the order status state machine, the customer status emails.
- Sanity schema changes — `studio/schemas/*.ts` plus the matching `backend/src/sanity.ts` type + helper + new route.
