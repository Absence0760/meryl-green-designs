...
- A component used in multiple places (`Button`, `Cart`, `PayFastRedirecting`) where consistency across surfaces matters.
...
- A **component name** (`Cart`, `Button`, `PayFastRedirecting`) — resolve via `find frontend/src/lib -maxdepth 1 -name "<name>.svelte"`.
